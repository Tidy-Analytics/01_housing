...
state_HU_data <- function(state_code, state_name) {
  ...
  # Read 2024 data (published July 2024)
  HU_24 <- data.table(
    dbGetQuery(
      conh,
      sprintf(
...
## LATEST, RELEASED SEPTEMBER 2025 (JULY 2025 VINTAGE)
  ...
  HU_25_jul <- setorder(HU_25_jul, -total_housing_units)

## NOVEMBER 2025 VINTAGE (RELEASED DECEMBER 2025)

  nov_file <- sprintf('./data/geo/%s_%s_AddressBlockCountList_122025.txt',
                      state_code, state_name)
  if (!file.exists(nov_file)) {
    stop(sprintf("November 2025 data file not found for %s (%s): %s",
                 state_name, state_code, nov_file))
  }

  HU_25_nov <- data.table(
    dbGetQuery(
      conh,
      sprintf(
    "SELECT * FROM read_csv_auto(
    './data/geo/%s_%s_AddressBlockCountList_122025.txt',
    header=True,
    normalize_names=True,
    all_varchar=True);",
    state_code, state_name)))[
  ...
  HU_25_jul <- HU_25_jul[!grepl("TOTAL", block_geoid)]
  HU_25_nov <- HU_25_nov[!grepl("TOTAL", block_geoid)]
  ...
  # Merge data from all vintages
  HU_merged <- merge(HU_20, HU_24, by = "block_geoid", all = TRUE)
  HU_merged <- merge(HU_merged, HU_25_jul, by = "block_geoid", all = TRUE)
  HU_merged <- merge(HU_merged, HU_25_nov, by = "block_geoid", all = TRUE)
...
# Note: Division by zero protection - returns NA for ratios/CAGRs when denominator is 0
