...
## compute county fips
hu_block[, county_fips := substr(block_geoid, 1, 5)]

hu_block_cbsa <- merge(hu_block, county_to_cbsa, by.x = "county_fips", by.y = "county", all.x = TRUE)

hu_block_cbsa <- as.data.table(
  merge(hu_block_cbsa, hu_cbsa, by.x = "cbsa23", by.y = "cbsa23", all.x = TRUE)
)
...
hu_block_state <- hu_block_state |> mutate(across(where(is.character), stringi::stri_enc_tonative))
hu_block_county <- hu_block_county |> mutate(across(where(is.character), stringi::stri_enc_tonative))
hu_block_cbsa <- hu_block_cbsa |> mutate(across(where(is.character), stringi::stri_enc_tonative))
hu_block_us <- hu_block_us |> mutate(across(where(is.character), stringi::stri_enc_tonative))
...
dbWriteTable(conh, "hu_block_state", hu_block_state, overwrite = TRUE)
